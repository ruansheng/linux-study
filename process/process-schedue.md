...
实时系统要求比完全公平调度更高
实时系统分类:
1.硬实时进程:必须保证在一定时间内完成，超时就会失败，后果很严重，典型例子有武器系统、医疗设备、航空航天系统、交通导航系统(linux不支持硬实时系统，有些修改版本支持)
2.软实时进程:硬实时系统的一种弱化形式，超过时间不会有灾难性后果，比如视频处理应用

linux提供了2种实时调度策略:先进先出(SCHED_FIFO)策略 和 时间片轮转(SCHED_RR)策略，无论使用那种，其优先级都高于完全公平调度

linux为实时进程提供了99个实时优先级，从0-99属于实时调度优先级，优先级的值越低，其优先级越高
每个CPU都有99个实时队列，对应着99中离散优先级，这样选择调度进程就简单了，只要选择一个优先级高的队列上的进程即可

一、先进先出(SCHED_FIFO)策略
没有时间片的概念，只要没有更高优先级的进程就绪，该进程就会一直执行，直到某种情况发生:
1. 自动放弃CPU自由，执行了阻塞或者调用了sched_yield
2. 进程终止了
3. 被一个更高优先级的进程抢占
如果进程调用了sched_yield让出CPU，那么该进程就会被放到对应队列的尾部
如果是被更高优先级的进程抢占，那么进程在队列中的位置不变，一旦高优先级的进程停止，被抢占的FIFO类型的进程会继续执行

二、时间片轮转(SCHED_RR)策略
具有相同优先级的进程轮流执行，每次都执行相同固定长度的时间片，直到某种情况发生:
1. 时间片耗尽
2. 进程自动放弃CPU，或者执行了阻塞，或者主动调用了sched_yield
3. 进程终止了
4. 被一个更高优先级的进程抢占
前2种情况进程会被放到其优先级队列的尾部，最后一种情况进程任然处于其运行队列的头部，在更高优先级进程执行完后，被抢占的进程继续执行，知道其时间片剩余的部分被耗光为止
POSIX提供了接口来查询SCHED_RR策略的时间片长度
#include<sched.h>
...
### 查看进程在哪个CPU上执行
```
ps -p 7214 -o pid,cmd,psr
```

### CPU亲和力
```
linux提供了非标准的系统调用来获取和修改进程的硬亲和力
#define _GUN_SOURCE
#include<sched.h>
